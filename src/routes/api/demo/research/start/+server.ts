...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		if (
			pState.credits < costCredits ||
			pState.metal < costMetal ||
			pState.crystal < costCrystal ||
			pState.fuel < costFuel
		) {
		...
		if (demoPlayer) {
			...
			demoPlayer.resources.credits =
				Number(demoPlayer.resources.credits ?? pState.credits) - costCredits;
			demoPlayer.resources.metal = Number(demoPlayer.resources.metal ?? pState.metal) - costMetal;
			demoPlayer.resources.crystal =
				Number(demoPlayer.resources.crystal ?? pState.crystal) - costCrystal;
