...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		if (!buildId)
			return new Response(JSON.stringify({ error: 'missing buildId' }), { status: 400 });
		...
		if (build.status !== 'queued' && build.status !== 'in-progress') {
			return new Response(JSON.stringify({ error: 'cannot cancel completed build' }), {
				status: 400
			});
		}
		...
		if (build.buildingId) {
			...
			if (buildingDef) {
				...
				const currentLevel =
					existingBuildings.find((b) => b.buildingId === build.buildingId)?.level ?? 0;
				...
				if (cost) {
					const [pState] = (await db
						.select()
						.from(playerState)
						.where(eq(playerState.userId, user.id))) as PlayerStateType[];
					if (pState) {
						await db
							.update(playerState)
							.set({
						...
						try {
							...
							if (demoPlayer) {
								...
								demoPlayer.resources.credits =
									Number(demoPlayer.resources.credits ?? pState.credits) + (cost.credits ?? 0);
								demoPlayer.resources.metal =
									Number(demoPlayer.resources.metal ?? pState.metal) + (cost.metal ?? 0);
								demoPlayer.resources.crystal =
									Number(demoPlayer.resources.crystal ?? pState.crystal) + (cost.crystal ?? 0);
								demoPlayer.resources.fuel =
									Number(demoPlayer.resources.fuel ?? pState.fuel) + (cost.fuel ?? 0);
