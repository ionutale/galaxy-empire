...
export const POST: RequestHandler = async ({ request }) => {
	...
	const [record] = await db
		...
		.where(eq(table.processedMissions.id, id));
	if (!record) return new Response(JSON.stringify({ error: 'not_found' }), { status: 404 });
	if (record.rolledBack)
		return new Response(JSON.stringify({ error: 'already_rolled_back' }), { status: 400 });
	...
	await db.transaction(async (ctx) => {
		...
		const state = (
			await ctx.select().from(table.playerState).where(eq(table.playerState.userId, record.userId))
		)[0];
		if (state) {
			await ctx
				.update(table.playerState)
				.set({
					credits: Math.max(0, state.credits - record.rewardCredits),
					metal: Math.max(0, state.metal - record.rewardMetal),
					crystal: Math.max(0, state.crystal - record.rewardCrystal)
				})
				.where(eq(table.playerState.userId, record.userId));
		}

		// restore lost ships back to player_ships
		if (record.quantityLost > 0) {
			const ships = await ctx
				.select()
				.from(table.playerShips)
				.where(eq(table.playerShips.userId, record.userId));
		...
		await ctx
			.update(table.processedMissions)
