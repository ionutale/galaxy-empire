...
export function resolveMission(shipRole: string | undefined, quantity: number): MissionResult {
	...
	if (!features.isFeatureEnabled('NEW_COMBAT')) {
		...
		const winChance = Math.min(
			cfg.maxWinChance,
			cfg.baseWinChance + roleWeight * cfg.winChancePerWeight
		);
	...
	const winChance = Math.min(
		cfg.maxWinChance,
		cfg.baseWinChance + roleWeight * cfg.winChancePerWeight + (Math.random() - 0.5) * 0.05
	);
	const outcome = Math.random() < winChance ? 'success' : 'failure';
	const lossPct =
		outcome === 'success'
			? cfg.lossPctOnSuccess * (1 - roleWeight * 0.02)
			: Math.min(0.9, cfg.lossPctOnFailure * (1 + roleWeight * 0.02));
	const quantityLost = Math.floor(quantity * lossPct);
	const survivors = Math.max(0, quantity - quantityLost);
