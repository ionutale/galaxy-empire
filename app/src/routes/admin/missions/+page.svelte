<script lang="ts">
	...
	async function load() {
		...
		error = '';
		try {
			const res = await fetch('/api/admin/processed-missions', {
...
<div class="mx-auto max-w-4xl space-y-4">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-muted text-sm">Review and rollback completed missions.</p>
		</div>
	...
	<section class="card p-4">
		<div class="flex items-center justify-between">
			...
			<div class="text-muted text-sm">{items.length} items</div>
		</div>
						...
						<div>
							{#if !m.rolledBack}
								<button
									...
									on:click={() => {
										if (confirm('Rollback mission?')) rollback(m.id);
									}}
