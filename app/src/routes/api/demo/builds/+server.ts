...
export const GET: RequestHandler = async () => {
  const builds = await db.select().from(table.buildQueue).where(eq(table.buildQueue.userId, 'demo_user'));
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const entry = { id, ...body, createdAt: new Date().toISOString(), status: 'queued', userId: 'demo_user' };
