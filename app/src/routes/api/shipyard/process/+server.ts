    ...
    try {
      ...
      await retryAsync(async () => {
        ...
        if (existing) {
          ...
          await db.insert(table.playerShips).values({ id: crypto.randomUUID(), userId: user.id, shipTemplateId: item.shipTemplateId, quantity: item.quantity });
        }
        await db.insert(table.processedBuilds).values({ id: crypto.randomUUID(), userId: user.id, shipTemplateId: item.shipTemplateId, quantity: item.quantity, processedAt: new Date() });
