  ...
  for (const item of completed) {
    try {
      ...
      await retryAsync(async () => {
        ...
        await db.insert(table.processedBuilds).values({ id: crypto.randomUUID(), userId: user.id, shipTemplateId: item.shipTemplateId, quantity: item.quantity, processedAt: new Date() }).run();
