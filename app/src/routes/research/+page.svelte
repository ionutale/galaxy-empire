...
<div class="p-4">
  <h2 class="text-2xl font-bold mb-4">Research Lab</h2>
            ...
            <div class="flex justify-between items-start">
              <div>
                <h3 class="card-title text-lg">
                  {t.name} 
                  <span class="badge badge-neutral ml-2">Lvl {level}</span>
                </h3>
            ...
            <div class="grid grid-cols-2 gap-2 text-sm mb-4">
              {#if cost.credits}
                <div class="flex justify-between">
                  ...
                  <span class={(playerState?.resources?.credits ?? 0) < cost.credits ? 'text-error font-bold' : ''}>{cost.credits}</span>
                </div>
              {/if}
              {#if cost.metal}
                <div class="flex justify-between">
                  <span class="opacity-70">Metal:</span>
                  <span class={(playerState?.resources?.metal ?? 0) < cost.metal ? 'text-error font-bold' : ''}>{cost.metal}</span>
                </div>
              {/if}
              {#if cost.crystal}
                <div class="flex justify-between">
                  <span class="opacity-70">Crystal:</span>
                  <span class={(playerState?.resources?.crystal ?? 0) < cost.crystal ? 'text-error font-bold' : ''}>{cost.crystal}</span>
                </div>
              {/if}
              {#if cost.fuel || cost.deuterium}
                <div class="flex justify-between">
                  <span class="opacity-70">Fuel:</span>
                  <span class={(playerState?.resources?.fuel ?? 0) < (cost.fuel || cost.deuterium) ? 'text-error font-bold' : ''}>{cost.fuel || cost.deuterium}</span>
                </div>
              {/if}
            </div>
